# Working with Halalatune as an AI agent

Halalatune is a 3rd party YouTube Music client for Android with halal content filtering, written in Kotlin. It follows material 3 design guidelines closely.
...
6. All string edits should be made to the `Halalatune/app/src/main/res/values/halalatune_strings.xml` file, NOT `Metrolist/app/src/main/res/values/strings.xml`. Do not touch other `strings.xml` or `halalatune_strings.xml` files in the project.
7. You are to follow best practices for Kotlin and Android development.

## Halal Filter Integration

Halalatune includes a unique halal content filter module (`:halalfilter`) that filters YouTube Music content based on Islamic principles. When adding new features:

1. Import and use the `HalalFilter` object from the `com.halalatune.halalfilter` package
2. The filter provides three levels: `STRICT`, `MODERATE`, `LIGHT`
3. Use `HalalFilter.isHalal()` to check if content passes the filter before displaying it
4. Add halal filter settings in the app's settings screen
...
3. Once the build is successful, you can test your changes on an emulator or a physical device. Install the generated APK located at `app/build/outputs/apk/foss/debug/app-universal-foss-debug.apk` and ask a human for help testing the specific features you worked on.

## Project Structure

- `:app` - Main Android app with Material 3 UI and halal filter integration
- `:innertube` - YouTube Music API wrapper
- `:kugou` / `:lrclib` / `:paxsenix` / `:shazamkit` / `:betterlyrics` - Lyrics providers
- `:lastfm` - Last.fm integration
- `:kizzy` - Discord Rich Presence
- `:halalfilter` - Halal content filtering module (unique to Halalatune)